...
/** Load zh-CN once and share the in-flight request across callers. */
let zhCNPromise: Promise<void> | null = null;

function ensureZhCN(): Promise<void> {
  if (zhCNPromise) return zhCNPromise;
  if (resources["zh-CN"]) return Promise.resolve();
...
/** Preload all zh-CN namespaces after the first paint. */
export function preloadZhCN(): Promise<void> {
  return ensureZhCN();
}
