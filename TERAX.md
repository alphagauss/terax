...
Workspace UI state is stored in `terax-workspace.<uuid>.json`. Global writable configuration uses the allowlisted `shared_store.rs` API, which performs lock/read-latest/minimal-mutation/atomic-replace writes. Entity catalogs mutate one record at a time. Writers notify their own process immediately, file watchers notify the other processes, and frontend subscribers coalesce notifications with a trailing revision read. Do not reintroduce a shared `LazyStore` or whole-list replacement for settings or entity catalogs.
