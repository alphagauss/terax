...
- **Unix** (`zshenv.zsh`, `zprofile.zsh`, `zlogin.zsh`, `zshrc.zsh`, `bashrc.bash`, `init.fish`) stores its generated shell integration under `~/.terax/shell-integration/`. Terax passes the relevant script explicitly when starting zsh, bash, or fish. Emit OSC 7 (cwd) and OSC 133 A/B/C/D (prompt boundaries + exit code) so the host can track cwd and detect command boundaries without re-parsing the prompt. Fish 4.0+ writes its own OSC 133 prompt markers; Terax sets `fish_features=no-mark-prompt`, sources its script with `-C`, and then re-asserts its own prompt to avoid doubling.
...
Each native Workspace window is a separate Terax application process. Every process owns one React webview plus its own Rust state, PTY, LSP, shell, file watcher, SSH/SFTP/tunnel manager, and live AI runtime. `workspace_process.rs` selects or creates a UUID Workspace at startup, holds its OS file lock for the process lifetime, and assigns UUID-specific Workspace and window-state files. `New Window`, `Ctrl/Cmd+Shift+N`, and the environment selector start another executable instance; they never reset the current window.

The bootstrap Local/WSL/SSH environment is immutable for that process. Environment initialization completes before tabs are warmed: the SSH status/host-key event bridge is registered first, WSL and SSH terminals remain cold until the target home is resolved, and an SSH bootstrap failure opens a credential/host-key recovery flow in the existing SSH window. Spaces are UI groups inside a Workspace; they store roots and tab state, not environments, and switching a Space never changes the process backend.

All Terax-owned files live under `~/.terax`: shared configuration, AI sessions, Workspace state, window state, SSH host keys, shell integration, and logs each use a dedicated child directory. Workspace UI state is stored in `workspaces/terax-workspace.<uuid>.json`. Global writable configuration uses the allowlisted `shared_store.rs` API, which performs lock/read-latest/minimal-mutation/atomic-replace writes. Entity catalogs mutate one record at a time. Writers notify their own process immediately, file watchers notify the other processes, and frontend subscribers coalesce notifications with a trailing revision read. Do not reintroduce a shared `LazyStore` or whole-list replacement for settings or entity catalogs.
...
- **tabs/** - presentational tab chrome shared by every Workbench group. Workbench state is the sole owner of tab order and activation. `useWorkspaceCwd` derives explorer root + inherited cwd for new tabs from the active page. `basename` splits on both `/` and `\`.
- **find/** - shared controlled React find and find/replace views plus the small `FindHandle` trigger contract. Search engines and query state remain in their owning editor, terminal, Git History, or future panel adapter.
- **header/** - top bar for workspace chrome, notifications, theme, settings, sidebars, and window controls. `WindowControls` rendered when `USE_CUSTOM_WINDOW_CONTROLS` is true (Linux + Windows; macOS uses native traffic lights).
...
- **settings/** - allowlisted shared settings store (`shared_store.rs`), preferences hook, settings window opener.
- **sidebar/** - primary activity rail plus independently resizable primary and secondary sidebars. The AI view owns the secondary sidebar today; collapsed view content stays unmounted, one view uses its own header, and multiple views get a top rail.
...
- **Sessions** (`lib/sessions.ts` + `store/chatStore.ts`): completed conversations are atomic snapshots at `sessions/<uuid>.json`; there is no shared index. A per-session OS run lock prevents two processes from continuing the same conversation. Streaming messages, approvals, tool state, and todos stay in the owning process, and `AgentRunBridge` publishes one complete snapshot after idle/error/stop. Other windows refresh metadata every three seconds or on focus and load full messages only when needed. `activeSessionId` is Workspace state, not global session state.
- **AI sidebar** (`components/AiSidebarPanel.tsx`): the single full chat surface. It owns session switching, messages, plan/todo/approval UI, and the fixed `AiSidebarComposer`; the status bar keeps one persistent AI entry that becomes the background-agent status pill while a run needs attention. The panel UI is lazy and unmounted while the secondary sidebar is collapsed, but runs continue through `AgentRunBridge`; per-session scroll positions are cached in memory and restored without animation when the panel remounts.
- **Composer** (`lib/composer.tsx`): root-mounted React context providing input state (text, attachments, voice) to `AiSidebarComposer`. It remains mounted while the sidebar is closed so drafts and attachments survive. Attachments include image, text-file, and `selection` kinds - selections come from `useChatStore.attachSelection(text, source)` (drained into chips, not pasted into the textarea) and are wrapped as `<selection source="terminal|editor">…</selection>` blocks at submit. Composer derives `isBusy` from `agentMeta.status` so it can mount safely before sessions hydrate.
