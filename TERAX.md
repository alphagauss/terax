...
Workspace UI state is stored in `terax-workspace.<uuid>.json`. Global writable configuration uses the allowlisted `shared_store.rs` API, which performs lock/read-latest/minimal-mutation/atomic-replace writes and emits file-change notifications to each process. Do not reintroduce a shared `LazyStore` for settings or entity catalogs.
