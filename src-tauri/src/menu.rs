...
#[cfg(target_os = "macos")]
use crate::modules::shared_store;
#[cfg(target_os = "macos")]
...
fn current_language<R: Runtime>(app: &AppHandle<R>) -> String {
    let _ = app;
    if let Ok(settings) = shared_store::shared_store_read("settings".to_string()) {
        if let Some(s) = settings.get("language").and_then(|value| value.as_str()) {
            return s.to_string();
        }
    }
    "en".to_string()
