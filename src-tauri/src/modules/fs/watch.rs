...
pub fn fs_watch_add(
    ...
    // SFTP has no portable server-push watcher. Remote mutations explicitly
    // refresh their parent directory; a polling watcher can be layered here
    // later without routing remote paths through the host filesystem.
    if workspace.is_ssh() {
        return Ok(());
    }
